...
nhanes_factor <- demog_cbc_chems %>%
  ...
  relocate(race_eth, .after = RIDRETH1) %>% #reorder the columns to keep the demographics together (mutate() automatically adds the new variable to the end of the dataset)
  relocate(sex, .after = RIASEX)
...
nhanes_educ2_3 <- nhanes_factor %>%
  mutate(education0 = case_when(DMDEDUC3 >= 0 & DMDEDUC3 <= 5 ~ "Less than 5th grade",
  ...
  mutate(education20 = case_when(DMDEDUC2 == 1 | DMDEDUC2 == 2 ~ "Less than high school",
                                 DMDEDUC2 == 3 ~ "High school or GED",
                                 DMDEDUC2 == 4 | DMDEDUC2 == 5 ~ "More than high school",
                                 DMDEDUC2 == 7 | DMDEDUC2 == 9 ~ "Unknown")) %>%
  mutate(education20 = na_if(education20, "Unknown")) #Make all the "Unknown" values missing

# Check the counts
table(nhanes_educ2_3$education0, useNA = "always")
table(nhanes_educ2_3$education20, useNA = "always")


# Merge the two education variables
nhanes_educ <- nhanes_educ2_3 %>%
  mutate(education = coalesce(education0,
                              education20))
...
nhanes_educ <- nhanes_educ %>%
  ...
  relocate(education0, .after = DMDEDUC2) %>%
  relocate(education20, .after = education0) %>%
  relocate(education, .after = education20)
...
nhanes_dataset <- nhanes_iron_categ %>%
  ...
  relocate(cut_groups, .after = RIDAGEYR)
