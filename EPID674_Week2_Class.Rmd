...
```{r load}
# Check file path
here("nhanes3.sas7bdat")

# Load data from SAS
nhanes <- read_sas(here("nhanes3.sas7bdat"))
colnames(nhanes)
```

```{r save_dataset}
# Save data as an R object
save(nhanes, file = here("nhanes3.rda"))
load(here("nhanes3.rda")) # Try reading in the data that you just saved

# Exporting data as a txt/csv file
write.table(nhanes, file = here("nhanes3.txt"))
write.csv(nhanes, file = here("nhanes3.csv"))

# Try reading in the .csv that you just made
nhanes.csv <- read.csv(here("nhanes3.csv"))
names(nhanes.csv)
```
...
nhanes <- nhanes %>% # This is the pipe operator
...
# Bind the variables that are useful to the nhanes data frame for use later
nhanes <- cbind(nhanes, AGE5b, AGE5c)

# Save the nhanes object you modified
save(nhanes, file = here("nhanes3.rda"))
